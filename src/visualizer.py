...
def _chart_chunking_comparison(df: pd.DataFrame, path: Path) -> None:
    ...
    for i, (col, label) in enumerate(zip(_CORE_METRICS, _CORE_LABELS)):
        ax.bar(
            x + i * width - (n - 1) * width / 2, grouped[col], width, label=label, color=palette[i]
        )
    ...
    ax.set_ylim(0, 1.1)
    ax.set_title(
        "Chunking Strategy Comparison (averaged over embed & retrieval)",
        fontsize=12,
        fontweight="bold",
    )
...
def _grouped_bar(
    ...
    for i, (_, row) in enumerate(grouped.iterrows()):
        ...
        ax.bar(
            x + offset,
            [row[m] for m in _CORE_METRICS],
            bar_width,
            label=str(row[groupby_col]),
            color=color,
        )
...
def _chart_mrr_heatmap(df: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("MRR Heatmap: Chunk Config × Embedding Model", fontsize=12, fontweight="bold")
